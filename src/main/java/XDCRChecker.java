 ...
 * XDCR Checker
 * -------------
 *	Application to help verify a uni-directional XDCR relationship between 2 clusters.
 *	Performs a check that each mutation on the source cluster makes it successfully to the destination cluster.
 *  	Works by opening a TAP stream on the source cluster to receive all of the mutations and checks:
 * 	(1) The key is available on the destination AND
 *	(2) The key on the destination has the same CAS value
 *
 * 	This class provides the entry point (main) function, and creates the couchbase client connection to both clusters.
 */
